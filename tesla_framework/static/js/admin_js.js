...
//        }
      ...
      //=================GOOGLE MAP=========================================
          ...
          if ($("#map-zoom").val())
              map_zoom = parseInt($("#map-zoom").val(), 10);
          else
              map_zoom = 2; 
          /**
          ...
          var image = '';
          if($('.map-icon input[type=radio]:checked').val())
              image = new google.maps.MarkerImage($('.map-icon input[type=radio]:checked').val(),
         ...
         $('.map-icon input[type=radio]').change(function(){
          ...
          function initialize() {
            ...
            autocompleteinput= document.getElementById('map_search')
            ...
            google.maps.event.addListener(autocomplete, 'place_changed', function() {
                ...
                if (!place.geometry) {
                  ...
                  autocompleteinput.className = 'notfound';
                  return;
                }
...
//                infowindow.setContent('<div><strong>' + place.name + '</strong><br>' + address);
           ...
           //Selecting the first option of autocomplete-----------------
                ...
                geocoder.geocode({"address":firstResult }, function(results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                     ...
                     // If the place has a geometry, then present it on a map.
                        ...
                        document.getElementById('marker-coords').value = latlng.lat() + "," + latlng.lng();
                        marker.setVisible(true);
                    }
                });   
             }
            ...
            //-------------------Autocomplete end-----------------------------------------------------------------------
            if ($("#marker-coords").val()){
                var coords = $("#marker-coords").val().split(',');
            ...
            google.maps.event.addListener(marker, 'dragend', function(evt){
                document.getElementById('marker-coords').value = evt.latLng.lat() + "," + evt.latLng.lng();
